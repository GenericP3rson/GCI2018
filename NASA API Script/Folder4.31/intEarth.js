...
var findFlybys= function(lat,lon,begin, callback) {
	var flyUrl = "https://api.data.gov/nasa/planetary/earth/assets?lon="+lon+"&lat="+lat+"&begin="+begin+"&api_key="+key;
...
findFlybys(lat, lon, begin, function(flybyJson) {
	...
	flybys["results"].forEach(function(r){
		var url = "https://api.data.gov/nasa/planetary/earth/imagery?api_key="+key
			+"&lat="+lat+"&lon="+lon+"&dim="+tileDimension+"&date="+r["date"].substring(0,10)+"&cloud_score=True";
